...
interface KidsMovieProps {
    ...
    onFethCharacterMovie: (item: kidsCharacterRequest & {
        characterId?: string;
        isCharacter?: boolean;
    }) => Promise<void>;
...
const fetchMovieNlCert = async (id: number | string) => {
    ...
    for (const t of preferTypes) {
        const found = nlList.find(
            (x: NlCertAge) => x?.type === t && x?.certification?.trim()
        );
...
export const useKidsMoiveStore = create<KidsMovieProps>((set, get) => ({
    ...
    onFetchTop10Movies: async () => {
        const res = await fetch(`https://api.themoviedb.org/3/discover/movie?api_key=${API_KEY}&language=ko-KR&with_genres=16&watch_region=KR&with_watch_providers=337&with_watch_monetization_types=flatrate&sort_by=popularity.desc&page=1`);
        const data = await res.json();
